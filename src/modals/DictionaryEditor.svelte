...
const addWordsFromFile = async (e: MouseEvent, defaultScore = 50, overwrite = true) => {
	let out = new Map();
...
const saveChanges = () => {
	...
	table.getData().forEach(row => {
		let [word, score, abbreviationFor] = [row.word, row.score, row.abbreviationFor];
...
<div class="page">
	<div class="controls">
		<div class="optionBox filters">
			<h3>⥸ Filter</h3>
			<p>Length</p>
			<RangeSlider min={1} max={40} step={1} range bind:values={wordLengthFilters} float />
			<p>Score</p>
			<RangeSlider min={0} max={100} bind:values={scoreFilters} step={1} range float />
		</div>
		<div class="optionBox words">
			<h3>+ Add</h3>
			<div class="row">
				<input type="text" bind:value={wordInput} placeholder="Word" />
				<input type="number" bind:value={scoreInput} min="0" max="100" step="1" />
		...
		<div class="optionBox selection" class:collapse={!selectedRows.length}>
			...
			<div class="row">
				<input type="number" bind:value={scoreAllInput} min="0" max="100" step="1" />
	...
	<div class="downloadButton" on:click={downloadDictionaryAsDict}>Save Changes</div>
</div>
<svelte:head>
	<link
