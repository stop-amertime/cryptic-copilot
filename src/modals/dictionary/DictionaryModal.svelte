<script lang="ts">
	...
	$: currentDictionarySize =
		Math.floor(currentDictionaryBlob.size / 1000).toLocaleString() + " KB";
	let replaceOrMerge = "merge";
	let newDictionaryName = "";
	$: changeDictionaryText = replaceOrMerge == "merge" ? "⥅ Merge" : "↷ Replace";
	let defaultScore = 50;
	let shouldOverwrite = true;
	const downloadCurrentDictionary = () =>
		saveBlobAs(currentDictionaryBlob, $dictionaryName);
...
<div class="tabRow">
	...
	<button on:click={() => (selectedTab = 2)} class:select={selectedTab == 2}
		>✎ Edit</button
	>
	<button on:click={() => (selectedTab = 3)} class:select={selectedTab == 3}
		>⭱ Top Words</button
	>
</div>
...
{#if selectedTab == 1}
	...
	<div class="manageTab">
		<div class="currentDictionary box">
			...
			<div class="download button" on:click={downloadCurrentDictionary}>
				↓ Download
			</div>
		</div>
