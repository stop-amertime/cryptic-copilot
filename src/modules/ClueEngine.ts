...
// Hashing { 
    ...
    const getHash = (word) => { return DICTIONARY.get(word)?.hash || hash(word) };

    const hashTable = {
        "E": 2, "A": 3, "R": 5, "I": 7, "O": 11, "T": 13, "N": 17, "S": 19, "L": 23, "C": 29, "U": 31, "D": 37, "P": 41,
        "M": 43, "H": 47, "G": 53, "B": 59, "F": 61, "Y": 67, "W": 71, "K": 73, "V": 79, "X": 83, "Z": 89, "J": 97, "Q": 101
    }
...
// Array Helpers { 
    ...
    function getLeftoverLetters(array1, subword) {        //? -> Remaining letters after a partial anagram.
        let array2 = Array.from(subword);
        if (typeof array1 == "string") { array1 = Array.from(array1) }
    ...
    function locationInSortedArray(element, array, comparer, start = 0, end = array.length) {
        if (array.length === 0)
            return -1;
        ...
        switch (c) {
            case -1: return locationInSortedArray(element, array, comparer, start, pivot);
            case 0: return pivot;
            case 1: return locationInSortedArray(element, array, comparer, pivot, end);
        };
    };
//}
// String Helpers {
    ...
    function findDirection(subword: string, baseword: string) : WordDirection {
        ...
        */
        ...
        switch (baseword) {
            case subword: return WordDirection.Forward;
            case [...subword].reverse().join(""): return WordDirection.Reverse;
            default: return WordDirection.Anagram;
        }
    }
...
export const DeviceFinder = {
    ...
    async searchDevices(targetword){         //== THE NEW DEVICE SEARCHER WITH PRIME-HASHING! 
    ...
    async fetchAndParseMWThesaurus(word) {
        ...
        for (let D of data)  // For each 'part' of speech (noun, verb...)
        {
            ...
            if (D.meta == undefined) { ThesaurusDTO = null; break; }

            // CHECK CORRECT WORD.
            let cleanword = toGridWord(D.meta.id);
            if (cleanword != word) { break; }
            ...
            for (let sensearray of D.def[0].sseq)  // For each 'sense' of the word (different definition)
            {
                ...
                if (senseobj.syn_list) // If uses syn_list (noun?)
                {
                    for (let syn of senseobj.syn_list[0]) // For each synonym
                    {
                        ...
                        if (senseobj.rel_list && senseobj.rel_list[index]) {
                            ...
                            synonyms.push({ "synonym": syn.wd, "related": related })
                            index++;
                        }
                    }
                }

                else if (senseobj.sim_list)   //IF uses sim_list (adjective?)
                {
                    for (let syn of senseobj.sim_list) {
                        synonyms.push({ 
                            "synonym": syn[0].wd, 
                            "related": syn.filter((x, index) => (index != 0)).map(x => x.wd) 
                            });
                    }
                ...
                senses.push({ "definition": senseobj.dt[0][1], "synonyms": synonyms })
                matches++;
            }
